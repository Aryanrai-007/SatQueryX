...
def build_pdf(query: str, plan: Any, evidence: Any, results: dict[str, Any], preview_png: bytes | None = None) -> bytes:
    ...
    story.append(Paragraph(f"Aggregate confidence: {evidence.confidence:.2f}", styles["BodyText"]))
    story.append(Paragraph("Sources: " + ", ".join(evidence.sources), styles["BodyText"]))

    if preview_png:
        story.extend([Spacer(1, 8), Paragraph("Input preview", styles["Heading2"]), RLImage(BytesIO(preview_png), width=170 * mm, height=105 * mm)])
    ...
    if "ndvi" in results:
        nd = results["ndvi"]
        rows += [["NDVI mean", f"{nd['mean']:.4f}"], ["Vegetated fraction", f"{nd['vegetated_fraction']:.2%}"]]
    ...
    if len(rows) > 1:
        story.extend([Spacer(1, 8), Paragraph("Metrics", styles["Heading2"]), Table(rows, colWidths=[70 * mm, 90 * mm], style=TableStyle([("BACKGROUND", (0,0), (-1,0), colors.lightgrey), ("GRID", (0,0), (-1,-1), 0.5, colors.grey), ("VALIGN", (0,0), (-1,-1), "TOP")]))])
    doc.build(story)
    return out.getvalue()
