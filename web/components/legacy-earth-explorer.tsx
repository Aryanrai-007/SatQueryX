...
export function LegacyEarthExplorer({ open, onClose, onSelect }: { open: boolean; onClose: () => void; onSelect: (lat:number, lon:number) => void }) {
  ...
  if (!open) return null;
  return <div className="fixed inset-0 z-[200] flex flex-col bg-[#05080d]">
    <header className="flex shrink-0 items-center justify-between border-b border-cyan-300/15 bg-[#071019]/95 px-4 py-3 backdrop-blur-xl">
    ...
    <div className="grid min-h-0 flex-1 lg:grid-cols-[1fr_310px]">
      <div className="relative min-h-0"><ExplorerMap center={center} onCenterChange={setCenter} drawing={drawing} onRoi={setRoi}/>
        <div className="absolute left-4 top-4 z-[500] flex max-w-[min(520px,calc(100%-32px))] flex-wrap gap-2">
          <div className="glass flex items-center gap-2 rounded-lg px-3 py-2"><Search size={13} className="text-cyan-300"/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search location…" className="w-44 bg-transparent text-xs text-white outline-none"/></div>
        ...
        <div className="absolute bottom-4 left-4 right-4 z-[500] flex flex-wrap items-center gap-2">
      ...
      <aside className="min-h-0 overflow-y-auto border-l border-white/8 bg-black/25 p-4">
        ...
        <div className="mt-5 border-t border-white/8 pt-4"><div className="text-[10px] font-semibold uppercase tracking-[.18em] text-slate-500">Temporal observation</div><div className="mt-3 rounded-lg border border-white/8 bg-white/[.025] p-3"><div className="flex justify-between text-xs"><span className="text-slate-300">Observation year</span><span className="font-mono text-cyan-200">{year}</span></div><div className="mt-2 text-[10px] leading-4 text-slate-600">Use the Earth workspace to navigate to an AOI, inspect imagery context, and then return the location to the SatQueryX mission pipeline.</div></div></div>
      </aside>
    </div>
  </div>;
}
